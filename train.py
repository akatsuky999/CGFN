...
def setup_logging(config):
    ...
    log_filename = f"TADNet_{now.strftime('%Y%m%d%H%M')}.log"
    best_model_name = f"BEST_CGFN_{now.strftime('%Y%m%d%H%M')}.pth"
...
def get_lr_scheduler(optimizer, config):
    ...
    min_factor = 1e-5 / initial_lr

    def lr_lambda(epoch):
        factor = 0.5 ** (epoch // config['train']['Reduction_step'])
...
def main(_Config):
    ...
    #     num_nodes,node_map='Data/PEMS03/PEMS03.txt'
    # )
    adj_mx, _ = generate_graph_matrices(
        config['data']['adj_file'],
        num_nodes
